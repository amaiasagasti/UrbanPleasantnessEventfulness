...
import sys
from .ThirdOctaveFilters import ThirdOctaveBandFilter, ThirdOctaveSPL, ThirdOctaveLevelTime
...
def mainEstacionario(data, rate, chunk):
    ...
    ThirdOctaveSPL_value = ThirdOctaveSPL(ThirdOctaveBands=ThirdOctave, 
                                        CHUNK=chunk, 
                                        RATE=rate, 
                                        TimeSkip=0)
    # Loudness
    loudness, specLoudness, _, _ = loudness_ISO532(ThirdOctaveSPL_value, SoundFieldDiffuse=0)
    ...
    # Cálculo sharpness
    sharpnessZwicker, sharpnessVonBismarck, sharpnessAures = calc_sharpness(loudness, specLoudness)
...
def mainVarianteTiempo(data, rate, chunk):
    ...
    # Loudness
    loudness, specLoudness = loudness_ISO532_time(ThirdOctaveLevelTime_value,SoundFieldDiffuse=0,RATE=rate,CHUNK=chunk)
    ...
    for i in range(len(loudness)):
        sharpnessZwickerTemp, sharpnessVonBismarckTemp, sharpnessAuresTemp = calc_sharpness(loudness[i],specLoudness[:, i])
